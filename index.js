...
function calculateExpression(expression) {
  //Преревіряємо чи аргумент є рядком
	return isNaN(expression) ? eval(expression) : "Argument is not a string";
...
function getNumberFromString(s) {
  ...
  return isNaN(parseFloat(s)) ? "Couldn't extract a number from the provided string." : parseFloat(s);
...
function getIntegerFromString(s) {
  ...
  return isNaN(parseInt(s)) ? "Couldn't extract a number from the provided string." : parseInt(s);
  // Перевіряємо, чи є отримане значення числом за допомогою isNaN
